...
public class LearnedSubjectService extends UniversityAbstractService<LearnedSubjectModel, LearnedSubjectEntity> {
    ...
    public List<LearnedSubjectModel> filterByStudentId(String studentId) {
        if (studentId != null && !studentId.isEmpty()) {
            return this.modelMap.values().stream().filter(obj -> studentId.equalsIgnoreCase(obj.getStudentId())).collect(Collectors.toList());
        }
        return null;
    }
    ...
    public List<LearnedSubjectModel> filterBySubjectId(String subjectId) {
        if (subjectId != null && !subjectId.isEmpty()) {
            return this.modelMap.values().stream().filter(obj -> subjectId.equalsIgnoreCase(obj.getSubjectId())).collect(Collectors.toList());
        }
        return null;
    }
